...
class AlignDlib:
    ...
    def put_text(self,image,landmark):
      ...
      return  image
    def simulateMask(self, rgbImg=None, mask_type=None,color=None,draw_landmarks=False, random_mask=True, bb=None, skipMulti=False, landmarks=None):
...
if __name__ == '__main__':
    ...
    paris=read_pairs_probe('pairs.txt')

    for probr, probd, probfiles in os.walk(source_folder):
        for pfile in probfiles:
            if(pfile in paris):
                img = cv2.imread(os.path.join(probr, pfile))
                im = dl.simulateMask(np.array(img, dtype=np.uint8), mask_type='a', color=(255, 0, 0), draw_landmarks=True)

...
if __name__ == '__main__':
    ...
    #img = dl.simulateMask(np.array(img, dtype=np.uint8), mask_type="a", color=(0, 0, 0), draw_landmarks=False,boundingbox=None)
    list_file=[]
    bbx=[]
    ...
    ftx=open("ftx.txt","w")
    logs=open("ijbc-log-6color.txt","w")
    for i in list_images:
        img_name=os.path.join("loose_crop",i)
        img = cv2.imread(img_name)
        idx=list_file.index(i)
        b=bbx[idx]
        img, maskType, color = dl.simulateMask(np.array(img, dtype=np.uint8), draw_landmarks=False,boundingbox=b)
        if not (img is None):
         cv2.imwrite(os.path.join("loose_crop_mask6",i),img)
